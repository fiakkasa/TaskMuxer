...
[BenchmarkCategory(nameof(InstanceTaskMultiplexer), "Polymorphic")]
[WarmupCount(2)]
[IterationCount(3)]
[RPlotExporter]
public class InstanceTaskMultiplexerPolymorphicBenchmarks
{
    ...
    private static readonly (int, string) result3 = (1, "hello");

    private static readonly dynamic result4 = new
    {
        Test = 1
    };
    ...
    private static Task AddCompletedWorkFuncFactory(InstanceTaskMultiplexer service, int index, string key, bool withItemKey = false)
    {
        ...
        return kind switch
        {
            ...
            {
                true => service.AddTask(new ItemKey(key, typeof(int)), GetCompletedWorkFuncFactory<int>(kind)),
                _ => service.AddTask(key, GetCompletedWorkFuncFactory<int>(kind))
            },
            ...
            {
                true => service.AddTask(new ItemKey(key, typeof(bool)), GetCompletedWorkFuncFactory<bool>(kind)),
                _ => service.AddTask(key, GetCompletedWorkFuncFactory<bool>(kind))
            },
    ...
    private static Task AddFuncFactory(InstanceTaskMultiplexer service, int index, string key, bool withItemKey = false)
    {
        ...
        return kind switch
        {
            0 => withItemKey switch
            {
                true => service.AddTask(new ItemKey(key, typeof(int)), GetFuncFactory<int>(kind)),
                _ => service.AddTask(key, GetFuncFactory<int>(kind))
            },
            1 => withItemKey switch
            {
            ...
            2 => withItemKey switch
            {
                true => service.AddTask(new ItemKey(key, typeof(bool)), GetFuncFactory<bool>(kind)),
                _ => service.AddTask(key, GetFuncFactory<bool>(kind))
            },
            3 => withItemKey switch
            {
